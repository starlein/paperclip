...
import { projects, projectGoals, goals, projectWorkspaces, workspaceRuntimeServices } from "@paperclipai_dld/db";
import {
  PROJECT_COLORS,
  deriveProjectUrlKey,
  isUuidLike,
...
async function ensureSinglePrimaryWorkspace(
  ...
  await dbOrTx
    .update(projectWorkspaces)
    .set({ isPrimary: true, updatedAt: new Date().toISOString() as any })
    .where(
      and(
        eq(projectWorkspaces.companyId, input.companyId),
        eq(projectWorkspaces.projectId, input.projectId),
