...
import { isGeminiUnknownSessionError, parseGeminiJsonl } from "@paperclipai_dld/adapter-gemini-local/server";
import { parseGeminiStdoutLine } from "@paperclipai_dld/adapter-gemini-local/ui";
  ...
  it("treats missing session messages as an unknown session error", () => {
    ...
    expect(
      isGeminiUnknownSessionError(
        "",
        'Error resuming session: Invalid session identifier "faa39838-14cf-4949-b1b3-c29a651722ac".',
      ),
    ).toBe(true);
