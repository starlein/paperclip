...
function maybeEnableUiDevMiddleware(entrypoint: string): void {
  ...
  if (normalized.endsWith("/server/src/index.ts") || normalized.endsWith("@paperclipai_dld/server/src/index.ts")) {
...
async function importServerEntry(): Promise<StartedServer> {
  ...
  // Production mode: import the published @paperclipai_dld/server package
  try {
    const mod = await import("@paperclipai_dld/server");
    return await startServerFromModule(mod, "@paperclipai_dld/server");
  } catch (err) {
    const missingSpecifier = getMissingModuleSpecifier(err);
    const missingServerEntrypoint = !missingSpecifier || missingSpecifier === "@paperclipai_dld/server";
    if (isModuleNotFoundError(err) && missingServerEntrypoint) {
      throw new Error(
        `Could not locate a Paperclip server entrypoint.\n` +
          `Tried: ${devEntry}, @paperclipai_dld/server\n` +
          `${formatError(err)}`,
      );
    }
    throw new Error(
