...
This specification is vendor-neutral. It is intended to be usable by any agent-company runtime, not only OhMyCompany.
...
- OhMyCompany must not require extra top-level fields for skill validity
- OhMyCompany-specific extensions must live under `metadata.paperclip` or `metadata.sources`
...
For OhMyCompany, the preferred fidelity extension is:
...
- issue/task OhMyCompany-only metadata
...
- OhMyCompany tools may emit the vendor extension by default as a sidecar while keeping the base markdown clean

Suggested OhMyCompany shape:
...
Additional rules for OhMyCompany exporters:
...
- omit empty and default-valued OhMyCompany fields when possible
...
- OhMyCompany exporters should emit `.paperclip.yaml` as a sidecar by default
...
## 19. OhMyCompany Mapping

OhMyCompany can map this spec to its runtime model like this:
...
- OhMyCompany extension:
  - `.paperclip.yaml` -> adapter config, runtime config, env input declarations, permissions, budgets, routine triggers, and other OhMyCompany-specific fidelity

Inline OhMyCompany-only metadata that must live inside a shared markdown file should use:

- `metadata.paperclip`

That keeps the base format broader than OhMyCompany.

This specification itself remains vendor-neutral and intended for any agent-company runtime, not only OhMyCompany.

## 20. Cutover

OhMyCompany should cut over to this markdown-first package model as the primary portability format.

`paperclip.manifest.json` does not need to be preserved as a compatibility requirement for the future package system.

For OhMyCompany, this should be treated as a hard cutover in product direction rather than a long-lived dual-format strategy.
