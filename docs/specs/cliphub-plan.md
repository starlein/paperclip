# ClipHub: Marketplace for OhMyCompany Team Configurations
...
> The "app store" for whole-company AI teams — pre-built OhMyCompany configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for OhMyCompany-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | OhMyCompany company (agents, projects, governance) |
...
A complete OhMyCompany company configuration:
...
Single-agent configurations designed to plug into a OhMyCompany org:
...
Portable skill files that any OhMyCompany agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with OhMyCompany's skill loading system
...
interface Listing {
  ...
  paperclipVersionMin: string;     // Minimum OhMyCompany version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target OhMyCompany company
...
| `POST` | `/api/listings/:id/install` | Install to OhMyCompany company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target OhMyCompany company (or create new)
  → ClipHub API calls OhMyCompany API to:
  ...
  → Redirect to OhMyCompany dashboard with new team running
```
...
### 5.3 Creator: Export from OhMyCompany → Publish

```
Running OhMyCompany company → "Export as Blueprint" (CLI or UI)
  → OhMyCompany exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (OhMyCompany brand blue/purple)
...
| Install flow | One-click deploy to OhMyCompany company |
...
{
  "targetCompanyId": "uuid",         // Existing OhMyCompany company
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as OhMyCompany
- **Backend**: Node.js API (or extend OhMyCompany server)
- **Database**: Postgres (can share OhMyCompany's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with OhMyCompany auth (or OAuth2)

### 9.2 Integration with OhMyCompany

ClipHub can be:
- **Option A**: A separate app that calls OhMyCompany's API to install blueprints
- **Option B**: A built-in section of the OhMyCompany UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing OhMyCompany UI and API.
...
- [ ] Install flow: blueprint → OhMyCompany company
...
- [ ] "Export from OhMyCompany" CLI command
