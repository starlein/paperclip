...
Add first-class **documents** to OhMyCompany as editable, revisioned, company-scoped text artifacts that can be linked to issues.
...
The `plan` key is conventional and reserved by OhMyCompany workflow/docs.
...
Update the OhMyCompany agent workflow so planning no longer edits the issue description.
