...
Audience: Operators setting up and running agents in OhMyCompany

## 1. What this system does

Agents in OhMyCompany do not run continuously.  
...
For local CLI adapters (`claude_local`, `codex_local`, `opencode_local`, `hermes_local`), OhMyCompany assumes the CLI is already installed and authenticated on the host machine.
...
OhMyCompany stores session IDs for resumable adapters.
...
OhMyCompany pushes runtime/activity updates to the browser in real time.
...
- If `ANTHROPIC_API_KEY` is set in adapter env or host environment, Claude uses API-key auth instead of subscription login. OhMyCompany surfaces this as a warning in environment tests, not a hard error.
