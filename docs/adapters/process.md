...
- Running a Python script that calls the OhMyCompany API
...
1. OhMyCompany spawns the configured command as a child process
2. Standard OhMyCompany environment variables are injected (`PAPERCLIP_AGENT_ID`, `PAPERCLIP_API_KEY`, etc.)
...
The script can use the injected environment variables to authenticate with the OhMyCompany API and perform work.
