import { asNumber, asString, parseJson, parseObject } from "@paperclipai_dld/adapter-utils/server-utils";
...
export function isGeminiUnknownSessionError(stdout: string, stderr: string): boolean {
  ...
  return /unknown\s+session|invalid\s+session(?:\s+identifier|\s+id)?|session\s+.*\s+not\s+found|resume\s+.*\s+not\s+found|checkpoint\s+.*\s+not\s+found|cannot\s+resume|failed\s+to\s+resume/i.test(
    haystack,
  );
}
