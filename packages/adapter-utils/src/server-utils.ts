...
export const DEFAULT_PAPERCLIP_AGENT_PROMPT_TEMPLATE = [
  ...
  "- Use child issues (subissues / sub-issues / sub issues, not related-task references) for parallel or long delegated work instead of polling agents, sessions, or processes. For tightly coupled code changes, serialize parent/child execution by reusing the parent workspace, blocking the parent, and serializing sibling sub-issues so only one runs at a time; start the next sibling only after the previous one is done or cancelled.",
...
export function renderPaperclipWakePrompt(
  ...
  const lines = resumedSession
      ? [
        ...
        "",
        "Execution contract: take concrete action in this heartbeat when the issue is actionable; do not stop at a plan unless planning was requested. Leave durable progress with a clear next action; use child issues (subissues / sub-issues / sub issues, not related-task references) instead of polling for long or parallel work; for tightly coupled code changes, reuse the parent workspace, block the parent, and serialize sibling sub-issues so only one runs at a time; start the next sibling only after the previous one is done or cancelled; and mark blocked work with the unblock owner/action.",
        "",
