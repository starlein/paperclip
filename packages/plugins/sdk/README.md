...
- **Worker SDK:** `@paperclipai_dld/plugin-sdk` — `definePlugin`, context, lifecycle
- **UI SDK:** `@paperclipai_dld/plugin-sdk/ui` — React hooks and slot props
- **Testing:** `@paperclipai_dld/plugin-sdk/testing` — in-memory host harness
- **Bundlers:** `@paperclipai_dld/plugin-sdk/bundlers` — esbuild/rollup presets
- **Dev server:** `@paperclipai_dld/plugin-sdk/dev-server` — static UI server + SSE reload
...
| Import | Purpose |
|--------|--------|
| `@paperclipai_dld/plugin-sdk` | Worker entry: `definePlugin`, `runWorker`, context types, protocol helpers |
| `@paperclipai_dld/plugin-sdk/ui` | UI entry: `usePluginData`, `usePluginAction`, `usePluginStream`, `useHostContext`, slot prop types |
| `@paperclipai_dld/plugin-sdk/ui/hooks` | Hooks only |
| `@paperclipai_dld/plugin-sdk/ui/types` | UI types and slot prop interfaces |
| `@paperclipai_dld/plugin-sdk/testing` | `createTestHarness` for unit/integration tests |
| `@paperclipai_dld/plugin-sdk/bundlers` | `createPluginBundlerPresets` for worker/manifest/ui builds |
| `@paperclipai_dld/plugin-sdk/dev-server` | `startPluginDevServer`, `getUiBuildSnapshot` |
| `@paperclipai_dld/plugin-sdk/protocol` | JSON-RPC protocol types and helpers (advanced) |
| `@paperclipai_dld/plugin-sdk/types` | Worker context and API types (advanced) |
...
```bash
pnpm add @paperclipai_dld/plugin-sdk
```
...
**Entity types** (for `entityTypes` on slots): `project` \| `issue` \| `agent` \| `goal` \| `run` \| `comment`. Full list: import `PLUGIN_UI_SLOT_TYPES` and `PLUGIN_UI_SLOT_ENTITY_TYPES` from `@paperclipai_dld/plugin-sdk`.
...
Full list in code: import `PLUGIN_CAPABILITIES` from `@paperclipai_dld/plugin-sdk`.

## UI quick start

```tsx
import { usePluginData, usePluginAction } from "@paperclipai_dld/plugin-sdk/ui";
...
import { usePluginAction, type PluginBridgeError } from "@paperclipai_dld/plugin-sdk/ui";
...
```tsx
import { useHostContext, usePluginData } from "@paperclipai_dld/plugin-sdk/ui";
import type { PluginDetailTabProps } from "@paperclipai_dld/plugin-sdk/ui";
...
```tsx
import { usePluginStream } from "@paperclipai_dld/plugin-sdk/ui";
...
Each slot type receives a typed props object with `context: PluginHostContext`. Import from `@paperclipai_dld/plugin-sdk/ui`.
...
```tsx
import type { PluginProjectSidebarItemProps } from "@paperclipai_dld/plugin-sdk/ui";
...
```tsx
import { useState } from "react";
import { usePluginAction, usePluginStream } from "@paperclipai_dld/plugin-sdk/ui";
...
```ts
import { createTestHarness } from "@paperclipai_dld/plugin-sdk/testing";
...
```ts
import { createPluginBundlerPresets } from "@paperclipai_dld/plugin-sdk/bundlers";
...
```ts
import { startPluginDevServer } from "@paperclipai_dld/plugin-sdk/dev-server";
