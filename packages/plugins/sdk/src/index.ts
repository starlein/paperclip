/**
 * `@paperclipai_dld/plugin-sdk` — Paperclip plugin worker-side SDK.
 *
 * This is the main entrypoint for plugin worker code.  For plugin UI bundles,
 * import from `@paperclipai_dld/plugin-sdk/ui` instead.
 *
 * @example
 * ```ts
 * // Plugin worker entrypoint (dist/worker.ts)
 * import { definePlugin, runWorker, z } from "@paperclipai_dld/plugin-sdk";
 *
...
// Manifest and constant types re-exported from @paperclipai_dld/shared
// Plugin authors import manifest types from here so they have a single
// dependency (@paperclipai_dld/plugin-sdk) for all plugin authoring needs.
export type {
 ...
 * @example
 * ```ts
 * import { z } from "@paperclipai_dld/plugin-sdk";
 *
