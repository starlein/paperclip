...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@paperclipai_dld/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@paperclipai_dld/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@paperclipai_dld/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@paperclipai_dld/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@paperclipai_dld/plugin-sdk/ui`
...
The `@paperclipai_dld/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
### 27.1 `@paperclipai_dld/plugin-test-harness`
...
```ts
import { createTestHarness } from "@paperclipai_dld/plugin-test-harness";
...
- `@paperclipai_dld/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@paperclipai_dld/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@paperclipai_dld/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@paperclipai_dld/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@paperclipai_dld/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@paperclipai_dld/plugin-sdk/ui`
...
- `@paperclipai_dld/plugin-test-harness`
