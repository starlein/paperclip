...
Inside this monorepo, the scaffold uses `workspace:*` for `@paperclipai_dld/plugin-sdk`.

Outside this monorepo, the scaffold snapshots `@paperclipai_dld/plugin-sdk` from the local Paperclip checkout into a `.paperclip-sdk/` tarball so you can build and test a plugin without publishing anything to npm first.
...
- `usePluginToast`
- `useHostContext`
- typed slot props from `@paperclipai_dld/plugin-sdk/ui`
